...
for(let i = 0; i < inventory.length; i++){
  //check if id match with 33
  if(inventory[i].id === 33 ){
    //output the car id, car year, car maker and, car model
...
let lastCar = 0;
//last car in the array, output in a string
console.log(` car maker: ${inventory[inventory.length -1].car_make} \n car model: ${inventory[inventory.length -1].car_model}`)
...
for(let i = 0; i < inventory.length; i++){
  //push the car_model from the inventory to carModels empty array
  carModels.push(inventory[i].car_model);
}

let carModelsSorted = [];
  //push the carModels which is bring sorted by alphabetically to carModelsSorted
 carModelsSorted.push(carModels.sort());
 //output the carModelsSorted
...
let carYears = [];
  //loop throught the inventory
  for(let i = 0; i < inventory.length; i++){
    //push all the car_years into carYears empty array
    carYears.push(inventory[i].car_year);
  }
//output the carYears arr
...
//loop throught the array of carYears
for(let i = 0; i < carYears.length; i++){
  //check if carYear is old then 2000
  if(carYears[i] < 2000){
    //only push if that car is less the 2000 to oldCars
    oldCars.push(carYears[i]);
  }
}
//output the oldCars length
...
//loop throught the inventory array of object
for(let i = 0; i < inventory.length; i++){
  //check if the car_model is Audi or BMW
  if(inventory[i].car_make === 'Audi' || inventory[i].car_make === 'BMW'){
    //push the object into empty BMWAndAudi array
    BMWAndAudi.push(inventory[i]);
  }
}

//output the BMWAndAudi in a JSON.stringify
